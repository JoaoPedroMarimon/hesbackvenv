...
def verificar_arquivojogo():
    with open("verijogo.txt", "r") as arquivo:
        conteudo = arquivo.read().strip()
    return conteudo


verijogo = verificar_arquivojogo()
...
def jogar_pedra_papel_tesoura(escolha_usuario):
    global pontuacao_usuario, pontuacao_computador, vida_usuario, vida_computador,verivida,veridamage
    ...
    label_vida_computador.config(text=f"Vida: {vida_computador}/100")
    ...
    if vida_usuario == 0 or vida_computador == 0:
        ...
        janela_batalha2.after(4000, mostrar_botoes)
...
def mostrar_vencedor():
    ...
    vencedor = "Você" if pontuacao_usuario == 4 else "Oponente"
...
def registrar_resultado(resultado):
    ...
    if verivida == 0:
        janela_batalha2.destroy()
    if vida_computador == 0:
        janela_batalha2.after(4000, janela_batalha2.destroy())
...
def abrir_dica():
    janela_dica2 = tk.Toplevel()
    janela_dica2.title("LEIALEIALEIA")
    janela_dica2.geometry("300x230")
    texto = '''SEGUINTE.
    ...
    BOA SORTE'''
    label = tk.Label(janela_dica2, text=texto, font=("Helvetica", 10), fg='red')
    label.pack(pady=20)


def executar_jogo2(n1, n2):
    global janela_batalha2, label_resultado, label_vida_usuario, label_vida_computador, botoes_opcao, imagem_tk3, imagem_tk4
    janela_batalha2 = tk.Tk()
    janela_batalha2.geometry("400x500")
    janela_batalha2.title("Batalha")

    botao_abrir_dica = tk.Button(janela_batalha2, text="LEIA", command=abrir_dica, font=("Helvetica", 10))
    ...
    label_resultado = tk.Label(janela_batalha2, text="", font=("Helvetica", 14))
    label_resultado.pack(pady=140)

    label_vida_computador = tk.Label(janela_batalha2, text=f"Vida: {vida_computador}/50", font=("Helvetica", 12))
    label_vida_computador.place(x=150, y=200)

    label_vida_usuario = tk.Label(janela_batalha2, text=f"Vida: {vida_usuario}/100", font=("Helvetica", 12))
    ...
    imagem_label3 = tk.Label(janela_batalha2, image=imagem_tk3)
    ...
    imagem_label4 = tk.Label(janela_batalha2, image=imagem_tk4)
    ...
    for opcao in opcoes:
        botao = tk.Button(janela_batalha2, text=opcao.capitalize(),
